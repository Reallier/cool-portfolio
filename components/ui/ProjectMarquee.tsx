  ...
  return (
    ...
    >
      ...
      <div className="relative rounded-3xl border border-border-subtle/80 bg-surface/80 backdrop-blur-md shadow-[0_22px_60px_rgba(15,23,42,0.25)]">
        <div
          className="flex gap-4 sm:gap-5 md:gap-6 px-4 sm:px-6 py-3 sm:py-4"
          style={{
            width: "max-content",
        ...
        >
          {duplicatedProjects.map((p, index) => (
            <Link
              ...
              className="group block flex-shrink-0 w-32 sm:w-36 md:w-40 lg:w-[22rem] xl:w-[26rem]"
              target={p.githubUrl ? "_blank" : "_self"}
              rel={p.githubUrl ? "noopener noreferrer" : ""}
            >
              <TiltCard className="relative flex h-full flex-col rounded-2xl sm:rounded-3xl border border-border-subtle/80 bg-gradient-to-br from-primary-blue/5 via-surface to-primary-cyan/5 p-2 sm:p-3 md:p-4 shadow-[0_18px_50px_rgba(15,23,42,0.24)] transition-all duration-300 hover:border-primary-blue/70 hover:shadow-[0_30px_90px_rgba(37,99,235,0.55)]">
                ...
                <div className="relative flex flex-col gap-4">
                  <div className="relative h-12 sm:h-14 md:h-16 lg:h-40 overflow-hidden rounded-xl sm:rounded-2xl bg-gradient-to-br from-primary-blue/25 via-primary-cyan/15 to-primary-blue/10">
                    ...
                    <div className="relative flex h-full items-center justify-center">
                      {p.githubUrl ? (
                        <div className="flex flex-col items-center gap-2 text-white">
                          <svg className="w-6 h-6 drop-shadow-[0_0_20px_rgba(15,23,42,0.45)]" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <span className="text-xs font-semibold tracking-wide text-white drop-shadow-[0_0_16px_rgba(15,23,42,0.6)]">
                          项目预览
                        </span>
                      )}
                    </div>
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    <h3 className="text-sm sm:text-base font-semibold text-text-main group-hover:text-primary-blue transition-colors">
                      {p.title}
                    </h3>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {p.topics && p.topics.length > 0
                        ? p.topics.slice(0, 3).map((topic: string) => (
                            <span
                              key={topic}
                              className="rounded-full bg-gradient-to-r from-primary-blue/15 to-primary-cyan/20 px-1.5 sm:px-2 py-0.5 text-[8px] sm:text-[10px] font-medium text-primary-blue shadow-[0_0_0_rgba(0,0,0,0)] transition-all duration-200 group-hover:shadow-[0_0_18px_rgba(37,99,235,0.55)]"
                            >
                              {topic}
                            </span>
                          ))
                        : p.role.map((r: string) => (
                            <span
                              key={r}
                              className="rounded-full bg-gradient-to-r from-primary-blue/10 to-primary-cyan/15 px-2 sm:px-3 py-0.5 sm:py-1 text-[10px] sm:text-xs font-medium text-text-soft shadow-[0_0_0_rgba(0,0,0,0)] transition-all duration-200 group-hover:shadow-[0_0_18px_rgba(37,99,235,0.45)]"
                            >
                              {r}
                            </span>
                          ))}
                    ...
                    {p.githubUrl && (
                      <div className="mt-1 flex flex-wrap items-center gap-2 sm:gap-3 text-[8px] sm:text-[10px] text-text-soft">
                        <div className="flex items-center gap-1">
                          <span className="h-1 w-1 sm:h-1.5 sm:w-1.5 rounded-full bg-gradient-to-r from-primary-blue to-primary-cyan shadow-[0_0_10px_rgba(37,99,235,0.9)]" />
